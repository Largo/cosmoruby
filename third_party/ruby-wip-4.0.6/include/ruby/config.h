...
/* cosmo: this was a flat "coroutine/amd64/Context.h", which is right for the
 * x86_64 half of a fat APE and catastrophic for the aarch64 half:
 * ruby.deps.mk already assembles coroutine/arm64/Context.S there, but cont.c
 * kept the amd64 struct layout (COROUTINE_REGISTERS 6, i.e. 48 bytes) while
 * the arm64 code writes 0xa0 bytes into it. Every Fiber -- and so every
 * Enumerator#next and Enumerator::Lazy -- segfaulted instantly on aarch64.
 * config.h is generated by configure on an x86_64 host and cannot know, so
