...
module RbConfig
  ...
  CONFIG["configure_args"] = " '--prefix=/tmp/ruby-cosmo-config' '--disable-shared' '--disable-pie' '--disable-install-doc' '--disable-install-rdoc' '--disable-install-capi' '--disable-yjit' '--disable-zjit' '--enable-load-relative' '--disable-dln' '--enable-debug-env' '--without-valgrind' '--without-jemalloc' '--without-gmp' '--with-parser=prism' '--with-thread=pthread' '--with-coroutine=amd64'"
