...
static const char kHexstr[] = "0123456789abcdef";
...
static bool AppendSanitizedComponent(char **p, char *end, const char *s) {
  unsigned char c;
  char *q = *p;
  while ((c = *s++)) {
    c = kToLower[c];
...
static bool BuildContextEntry(char buf[hasatleast PATH_MAX],
                              const char *source, const char *include) {
  char *p = buf;
