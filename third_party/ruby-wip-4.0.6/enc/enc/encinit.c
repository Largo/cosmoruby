...
#define init_enc(name) init(Init_##name, "enc/"#name DLEXT)
#define init_trans(name) init(Init_trans_##name, "enc/trans/"#name DLEXT)
#define provide(func, name) { \
    extern void Init_##func(void); \
    Init_##func(); \
    rb_provide(name DLEXT); \
}
...
Init_enc(void)
{
    ...
    init_enc(windows_1253);
    init_enc(windows_1254);
    init_enc(windows_1257);
